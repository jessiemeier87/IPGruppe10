...
//GitHub mit alte Versionen: https://github.com/jessiemeier87/IPGruppe10.git
// Videos: https://drive.google.com/drive/folders/191i_mBmXR0LZ_FxVIbM9cxATVisSYGud?usp=sharing
...
//Variable für die Funktion scroll()
...
void scrollText(byte *textToScroll[], int n) {
  ...
  for (int i = 0; i < 8; i++) {
    ...
    /*
     * scrollIndex hält fest, wo wir uns im Text befinden, der über den 
     * Bildschirm scrollt, so dass wir darauf zurückkommen können, wenn
     * distanceSensorOutput() ausgelöst wird
     */
    if ((scrollIndex-n) < 0) {
      text[i] = text[i] >> CHAR_WIDTH * ((scrollIndex-n)*(-1));
    } else {
      text[i] = text[i] << CHAR_WIDTH * (scrollIndex-n);
    }
  }
  displayText(text);
  scrollIndex+=1;
...
void getCO2() {
  ...
  // kein unterer Grenzwert, da es keinen Mindestwert für CO2 gibt
  if (eCO2 >= 1000) {
    CO2InRange = false;
  } else {
      CO2InRange = true;
   ...
   * Lichtsensor. Deshalb wenn ich (Jessica) aus Versehen den Lichtsensor von meiner
   * Platine abgezogen habe, um den von Herrn Amanuel zu testen, ohne die Platine vorher
   * abzustecken, druckte er immer noch Werte in die serielle Schnittstelle. Ich glaube, es
   * war ein Floating Pin.
  */
...
void distanceSensorOutput() {
  ...
  } else if (distance > 20 && !alarmOn && !testBuzzer && !testRedLED && !testGreenLED && !testYellowLED) {
      withinDist = false;
      digitalWrite(RED_OUTPUT, LOW);
      digitalWrite(YELLOW_OUTPUT, LOW);
      digitalWrite(GREEN_OUTPUT, LOW);
...
/* 
 *  Steuert, welcher Text auf der LED-Matrix angezeigt wird, zusammen mit displayNums()
 *  Scroll() erhöht displayCounter und verfolgt, ob die Taste gedrückt wurde oder nicht.
 */
void scroll() {
  if (displayCounter == 1 &&  (scrollIndex >= 15 || buttonToggle)  && !alarmOn) {
    displayCounter += 1;
    buttonToggle = false;
      ...
      displayCounter += 1;
      scrollIndex = 0;
  } else if (displayCounter == 3 && (scrollIndex >= 15 || buttonToggle) && !alarmOn) {
    displayCounter += 1;
    buttonToggle = false;
      ...
      displayCounter += 1;
      scrollIndex = 0;
  } else if (displayCounter == 5 &&  (scrollIndex >= 15 || buttonToggle)  && !alarmOn) {
    displayCounter += 1;
    buttonToggle = false;
  } else if (displayCounter == 6 && buttonToggle == true && !alarmOn) {
      buttonToggle = false;
      displayCounter += 1;
      scrollIndex = 0;
  } else if (displayCounter == 7 &&  (scrollIndex >= 15 || buttonToggle)  && !alarmOn) {
    displayCounter += 1;
    buttonToggle = false;
  } else if (displayCounter == 8 && buttonToggle == true && !alarmOn) {
      buttonToggle = false;
      displayCounter = 1;
      scrollIndex = 0;
  }
...
//Steuert, welcher Text auf der LED-Matrix angezeigt wird, zusammen mit scroll()
void displayNums() {
  /*
   * Verzögerung bei der Entfernungserkennung, so dass < Zeit >
   * immer am Anfang über die LED-Matrix scrollt, auch wenn
   * sich ein Objekt innerhalb von 20 cm befindet
   */
